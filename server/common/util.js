...
// password 해쉬 생성
const makeHashedPassword = (plainPassword, salt) => {
  return new Promise((resolve, reject) => {
    const hashSalt = salt || crypto.randomBytes(64).toString('hex');
    crypto.pbkdf2(plainPassword, hashSalt, 3708, 64, 'sha512', (err, key) => {
      if (err) reject(err);
      resolve({ hashPassword: key.toString('hex'), hashSalt });
    });
  });
};
