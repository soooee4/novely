...
const Header = () => {
  ...
  return (
    <Whole>
        ...
        <MenuBtnBox>
          ...
          {!profile.login_id ? (
            <Buttons
              ...
              color={"white"}
              width={83}
              setModalOpen={() => dispatch(setModalOpen({ open: true, content: "login", width: 550, height: 340 }))}
            />
          ) : (
            // 일반 유저 로그인 상태
            <>
              ...
              <Buttons
                ...
                setModalOpen={() => dispatch(setModalOpen({ open: true,  content: "editProfile", width: 400, height: 600 }))}
                minWidth={70}
              />
              <Buttons
                type={CODE.BUTTON.BORDER}
      ...
      <ModalPopup width={modalWidth(content)} height={modalHeight(content)} />
    </Whole>
  );
};
