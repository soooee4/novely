...
// 	const profile = JSON.parse(localStorage.getItem("profile"));        // 로컬스토리지에 저장된 사용자 정보
...
// 					marginTop: 4,
// 					marginBottom: 2,
// 					backgroundImage: `url(${process.env.REACT_APP_IMAGE_DIRECTORY}/${profile.image})`,  // url 자리에 현재 로컬 ip주소 입력
		...
		<Wrapper>
			<Box
				sx={{
					...
					marginTop: 4,
					marginBottom: 2,
					// backgroundImage: `url(http://192.168.200.12:8080/profile/${profile.image})`,  // url 자리에 현재 로컬 ip주소 입력
					backgroundImage: `url(${process.env.REACT_APP_IMAGE_DIRECTORY}/${profile.image})`,
					backgroundSize: "cover",
				}}
			/>
			{/* 파일 하나만 선택하도록 할 것이기 때문에 files의 0번째 배열을 직접적으로 가져옴 */}
