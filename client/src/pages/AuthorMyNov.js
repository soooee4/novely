...
// 					setDescription={(data) => setDescription(data)} // 얘도 안고쳤고 
// 					regditMainNovData={regditMainNovData}
// 					profile={profile}
//           postMainNovel={() => postMainNovel()}  // 여기 생긴거 다르고 이겄때문에 2번 포스트 될것임 
// 				/>
// 			);
// 		}
// 	};
...
const AuthorMyNov = () => {
  ...
  // 미완성 소설 중 로그인한 계정이 created user인 작품만 가져오기
  useEffect(() => {
    getData("novel/getAuthorNovel", { created_user: profile.login_id, login_id: profile.login_id })
      .then(function (data) {
        setInCompleteNovData(data);
      })
      .catch((err) => {
